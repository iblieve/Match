...
class XGEODistanceStrategy(object):
    ...
    def compare(p_address_a:XAddress=None, p_address_b:XAddress=None) -> (bool, float):
        ...
        lat_diff = rad(p_address_a.latitude) - rad(p_address_b.latitude)
        lon_diff = rad(p_address_a.longitude) - rad(p_address_b.longitude)
        s = 2 * math.asin(math.sqrt(math.pow(sin(lat_diff / 2), 2) + cos(rad(p_address_a.latitude)) * cos(rad(p_address_b.latitude)) * math.pow(sin(lon_diff / 2), 2)))
